...
4. Update system paths in `quantified_self/.env`
...
# upload the file
with open(os.getenv("LOCAL_ENV_FILE"), "rb") as f:
    content = f.read()
...
resp = requests.post(
    f"{os.getenv('DATABRICKS_HOST')}/api/2.0/workspace/import",
    headers={"Authorization": f"Bearer {os.getenv('DATABRICKS_PAT')}"},
    json=payload,
    timeout=2,
)
