...
def stg_body_measurements():
    ...
    raw_body_measurements = utils.convert_column_values_to_lower_case(
        raw_body_measurements, ["is_pump"]
    )

    raw_body_measurements = utils.preserve_timezone(raw_body_measurements, "date_time")

    raw_body_measurements = raw_body_measurements.withColumnRenamed(
        "tond", "belly"
    ).withColumnRenamed("ass", "glutes")

    raw_body_measurements = utils.type_cast_columns(
        raw_body_measurements, ["date_time"], "timestamp"
    )
    ...
    raw_body_measurements = utils.type_cast_columns(
        raw_body_measurements, column_list=float_columns, column_type="float"
    )

    stg__body_measurements = raw_body_measurements.dropDuplicates().fillna(0, subset=float_columns)

    stg__body_measurements = utils.prepare_for_export(
        dataframe=stg__body_measurements,
        timestamp_column="date_time",
        source_name="body_measurements",
    )
