...
def calendar():
    ...
    calendar = (
        calendar
        .withColumn("calendar_year", sf.year("date"))
    ...
    calendar = (
        calendar
        .withColumn("is_weekend", sf.col("day_of_week").isin("Sat", "Sun"))
        ...
        .withColumn("is_quarter_end", sf.expr("date = add_months(trunc(date, 'quarter'), 3) - interval 1 day"))
    ...
    calendar = (
        calendar
        .withColumn("week_start_date", sf.date_sub("date", sf.dayofweek("date") - 2))
    ...
    calendar = (
        calendar
        .withColumn("next_day", sf.date_add("date", 1))
    ...
    calendar = (
        calendar
        .withColumn("is_current_day", sf.col("date") == constants.CURRENT_DATE)
    ...
    calendar = (
        calendar
        .withColumn("age_years", sf.floor(sf.months_between("date", sf.lit(constants.DATE_OF_BIRTH)) / 12))
        .withColumn("age_months", sf.floor(sf.months_between("date", sf.lit(constants.DATE_OF_BIRTH))))
